...
class NGA(Source):
    ...
    def is_nga_file_update_needed(self):
        ...
        file_dates = get_file_creation_date(Constants.NGA_ZIP_FILE_PATH)
        creation_date = file_dates[0]
        altered_date = file_dates[1]
        ...
        print(
            ...
            creation_date.year,
            get_month_name_from_int(
                Constants.months_abbreviated, creation_date.month),
            creation_date.day,
        )
        ...
        if creation_date < altered_date:
            print(
                ...
                altered_date.year,
                get_month_name_from_int(
                    Constants.months_abbreviated, altered_date.month),
                altered_date.day,
            )
        ...
        for file in csv_file_names:
            ...
            if "published_images.csv" in end_part:
                df = pd.read_csv(file, on_bad_lines='skip', index_col=False,
                                 dtype='unicode')
                df.rename(
                    columns={'depictstmsobjectid': 'objectid'}, inplace=True)
                df.to_csv(file)
    ...
    def extract_zip_and_fix(self):
        ...
        if os.path.exists(Constants.NGA_ZIP_FILE_PATH):
            ...
            with ZipFile(Constants.NGA_ZIP_FILE_PATH, "r") as zip_f:
                ...
                if zip_f.namelist():
                    os.rename(
                        Const.NGA_CSV_DIR + Const.NGA_FOLDER_TO_RENAME, Const.NGA_FOLDER_RENAME_TO)
                    print("Now cleaning up the files in",
                          Const.NGA_CSV_CONTAINER)
